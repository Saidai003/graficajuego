...
class D{ //Directorio
    ...
    void RecursiveNav(D *current, int cursorX, int cursorY){
    ...
    printf("%c", current->name[j-5]);
                ...
                for(int z = current->dires->size() - 1; z >= 0; --z){
                    if(i > 3 && z == (i - 4) && j > 1 && (j-2) < (*current->dires)[z].name.size()){
                        printf("%c", (*current->dires)[z].name[j-2]); //nuevo, no lo sabia
                        printed = true;
                        break;
                    }
                }
                if(!printed){
                    for(int b = current->files->size() - 1; b >= 0; --b){
                        if(i > (3 + current->dires->size()) && b == (i - 4 - current->dires->size()) && j > 0 && (j-2) < current->dires->size()){
                            printf("%c", (*current->files)[b].name[j-2]);
                            printed = true;
                            break;
                        }
                    }
    ...
    printf("\n");
        ...
        if(input == 'e' && cursorX == 3 && cursorY == 0){
            try{
                ...
                //(*current->dires)[z].
                (*current->dires).emplace_back(newdir); // Añadir el nuevo directorio a 'files'
              ...
              for(int z = (*current->dires).size() - 1; z >= 0; --z){
                auto it = current->dires[z];
                        if(input == 'e'  && cursorY-4 == z && cursorX > 0){
                            try{
                                   printf("\033[H\033[J");
                               RecursiveNav(current.dires, cursorX, cursorY);
